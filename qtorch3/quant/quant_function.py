...
__all__ = ["fixed_point_quantize", "block_quantize", "float_quantize", "quantizer", 
           "float_quantize_inf",
           "float_quantize_custom", "quantizer_custom",
           "ratio_abs_leq_thrs", "ratio_abs_geq_thrs", "ratio_abs_leq_geq_thrs"]
#-----
...
#----- 
# NOTE: return +-inf if overflow occurs, unlike float_quantize().
def float_quantize_inf(x, exp, man, rounding="stochastic"):
    """
    Quantize a single precision Floating Point into low-precision Floating Point
    ...
    #----- 
    if rounding == "nearest":
        out = quant_module.float_quantize_nearest_inf(x.contiguous(), man, exp)
    elif rounding == "stochastic":
        raise ValueError
    #-----
    return out

def float_quantize_custom(x, exp, man, exp_bias_pow, allow_inf):
    assert isinstance(x, torch.Tensor), "x is not a single precision Floating Point Tensor"
    quant_module = get_module(x)
    y = quant_module.float_quantize_nearest_custom(x.contiguous(), man, exp, exp_bias_pow, allow_inf)
    return y

def quantizer_custom(fwd_num):
    if fwd_num != None:
        assert isinstance(fwd_num, FloatingPoint)

    forward_quant = (
        lambda x, quant_module, exp_bias_pow, allow_inf:
        quant_module.float_quantize_nearest_custom(x.contiguous(), fwd_num.man, fwd_num.exp, exp_bias_pow, allow_inf)
    )
    ...
    class Rounding(torch.autograd.Function):
        ...
        def forward(self, x, exp_bias_pow, allow_inf):
            ...
            if fwd_num == None:
                ...
                quant_module = get_module(x)
                y = forward_quant(x, quant_module, exp_bias_pow, allow_inf)
        ...
        def backward(self, gy):
            # set: g{x, exp_bias_pow, allow_inf}.
            if self.needs_input_grad[0]:
                gx = gy
            else:
                gx = None
            gexp_bias_pow, gallow_inf = None, None

            # ret.
            return (gx, gexp_bias_pow, gallow_inf)

    return Rounding.apply

def ratio_abs_leq_thrs(x, thrs):
    assert isinstance(x, torch.Tensor), "x is not a single precision Floating Point Tensor"
    quant_module = get_module(x)
    ratio = quant_module.ratio_abs_leq_thrs(x.contiguous(), thrs)
    # ratio = ratio.sum() / x.numel()
    return ratio

def ratio_abs_geq_thrs(x, thrs):
    assert isinstance(x, torch.Tensor), "x is not a single precision Floating Point Tensor"
    quant_module = get_module(x)
    ratio = quant_module.ratio_abs_geq_thrs(x.contiguous(), thrs)
    # ratio = ratio.sum() / x.numel()
    return ratio

def ratio_abs_leq_geq_thrs(x, thrs_l, thrs_g):
    assert isinstance(x, torch.Tensor), "x is not a single precision Floating Point Tensor"
    quant_module = get_module(x)
    ratio = quant_module.ratio_abs_leq_geq_thrs(x.contiguous(), thrs_l, thrs_g)
    # ratio = ratio.sum() / x.numel()
    return ratio
#-----
