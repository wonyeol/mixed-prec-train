...
//----- 
// NOTE: return +-inf if overflow occurs, unlike clip_exponent(...).
__device__ __forceinline__ unsigned int clip_exponent_inf(int exp_bits, int man_bits,
                                                          unsigned int old_num,
                                                          unsigned int quantized_num) {
  if (quantized_num == 0)
    return quantized_num;
  ...
  if (quantized_exponent_store > max_exponent_store)
  {
    //----- 
    unsigned int old_sign = old_num >> 31 << 31;
    quantized_num = old_sign | 0x7f800000; //= sgn(old_num) * inf (in FP32).
    //-----
  }
  return quantized_num;
}
